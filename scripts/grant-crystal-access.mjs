/**
 * One-time script: Grant download access for Crystal Ceri (Crystalceri25@gmail.com)
 * Product: From Sonographer to CEO eBook (digital_products.id = 1)
 * Run: node scripts/grant-crystal-access.mjs
 */
...
const CUSTOMER_EMAIL = 'Crystalceri25@gmail.com';
const CUSTOMER_NAME = 'Crystal Ceri';
const PRODUCT_ID = 1;
...
async function run() {
  ...
  if (existing.length > 0) {
    ...
    // Create user with a reset token for password setup
    resetToken = crypto.randomBytes(32).toString('hex');
    const resetExpiry = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000); // 7 days
    
    await conn.execute(
      `INSERT INTO users (email, name, firstName, lastName, role, membership_tier, created_at, updated_at, password_reset_token, password_reset_expires)
       VALUES (?, ?, ?, ?, 'user', 'free', NOW(), NOW(), ?, ?)`,
      [CUSTOMER_EMAIL, CUSTOMER_NAME, 'Crystal', 'Ceri', resetToken, resetExpiry]
    );
    const [newUser] = await conn.execute('SELECT id FROM users WHERE email = ? LIMIT 1', [CUSTOMER_EMAIL]);
    userId = newUser[0].id;
    isNew = true;
    console.log(`Created new user: id=${userId}`);
  }

  // 2. Grant digital purchase access
  ...
  if (existingPurchase.length > 0) {
    console.log(`Download access already granted for user ${userId}, product ${PRODUCT_ID}`);
  } else {
    await conn.execute(
      'INSERT INTO digital_purchases (user_id, product_id, stripe_checkout_session_id, purchased_at) VALUES (?, ?, ?, NOW())',
      [userId, PRODUCT_ID, 'manual-grant-crystal-ceri']
    );
    console.log(`Granted download access: user ${userId}, product ${PRODUCT_ID}`);
  }

  // 3. Update funnel_purchases to link user_id
  await conn.execute(
    "UPDATE funnel_purchases SET user_id = ? WHERE email = ? AND user_id IS NULL",
    [userId, CUSTOMER_EMAIL]
  );
  console.log(`Updated funnel_purchases to link user_id=${userId}`);

  // 4. Get the download file URL
  const [productRows] = await conn.execute(
    'SELECT id, title, slug FROM digital_products WHERE id = ? LIMIT 1',
    [PRODUCT_ID]
  );
  ...
  // 5. Send access email via SendGrid
  if (SENDGRID_API_KEY) {
    const emailHtml = `
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>Your eBook is Ready</title></head>
<body style="font-family: Inter, Arial, sans-serif; background: #f8fafc; margin: 0; padding: 20px;">
  <div style="max-width: 600px; margin: 0 auto; background: white; border-radius: 12px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.08);">
    ...
    <div style="padding: 40px;">
      <h2 style="color: #0e4a50; margin: 0 0 16px;">Hi Crystal! Your eBook is ready 🎉</h2>
      <p style="color: #475569; line-height: 1.6; margin: 0 0 24px;">
        Thank you for your purchase of <strong>${PRODUCT_NAME}</strong>. 
        ${isNew ? 'We\'ve created your All About Ultrasound™ account and your download is ready.' : 'Your download access has been activated.'}
      </p>
      ...
      <div style="background: #f0fbfc; border: 1px solid #b2e8eb; border-radius: 8px; padding: 20px; margin: 0 0 24px;">
        ...
        <a href="${downloadUrl}" style="display: inline-block; background: #189aa1; color: white; text-decoration: none; padding: 12px 24px; border-radius: 8px; font-weight: 600; font-size: 15px;">
          Download Your eBook →
        </a>
      </div>
      ...
      <div style="background: #fff7ed; border: 1px solid #fed7aa; border-radius: 8px; padding: 20px; margin: 0 0 24px;">
        ...
        <p style="margin: 0 0 12px; color: #7c2d12; font-size: 14px;">Click below to set your password and get full access to your All About Ultrasound™ account.</p>
        <a href="${setPasswordUrl}" style="display: inline-block; background: #ea580c; color: white; text-decoration: none; padding: 10px 20px; border-radius: 8px; font-weight: 600; font-size: 14px;">
          Set Your Password
        </a>
      </div>
      ` : ''}
      ...
      <p style="color: #94a3b8; font-size: 13px; margin: 24px 0 0;">
        If you have any questions, reply to this email or contact us at support@allaboutultrasound.com
      </p>
    </div>
    ...
    if (response.ok || response.status === 202) {
      console.log(`✅ Access email sent to ${CUSTOMER_EMAIL}`);
    } else {
      const body = await response.text();
      console.error(`❌ Failed to send email: ${response.status} ${body}`);
    }
  } else {
    console.warn('No SENDGRID_API_KEY — skipping email send');
  }
