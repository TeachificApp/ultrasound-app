...
import {
  activatePreparedUserSession,
  confirmUserSessionReplacement,
  issueApprovedUserSession,
  prepareUserSession,
...
export function registerAuthLoginRoute(app: Express) {
  ...
  app.post("/api/auth/login", async (req: Request, res: Response) => {
    try {
      const { email, password, host, deviceId } = req.body ?? {};
      ...
      // Check the active-device policy before issuing any cookie. A second
      // ordinary-user browser must be confirmed by the account holder first.
      const openId = await ensureUserOpenId(db, user);
      const preparedSession = await prepareUserSession(
        db,
        user,
      ...
      await activatePreparedUserSession(
        db,
        user,
  ...
  /** Complete a short-lived user-confirmed replacement of another device session. */
  app.post("/api/auth/confirm-session-replacement", async (req: Request, res: Response) => {
    try {
      const { sessionReplacementToken, host } = req.body ?? {};
      if (typeof sessionReplacementToken !== "string" || !sessionReplacementToken) {
        return res.status(400).json({ error: "Session confirmation is required." });
      }
      const db = await getDb();
      if (!db) return res.status(503).json({ error: "Service temporarily unavailable." });

      const replacement = await confirmUserSessionReplacement(db, sessionReplacementToken);
      if (!replacement) {
        return res.status(401).json({ error: "This sign-in confirmation has expired. Please sign in again." });
      }
      const [user] = await db.select().from(users).where(eq(users.id, replacement.userId)).limit(1);
      ...
      console.error("[authLogin] Session replacement error:", err);
  ...
  app.get("/api/auth/magic-verify", async (req: Request, res: Response) => {
    const { token, returnTo, host: hostParam, deviceId } = req.query as Record<string, string>;
    ...
    try {
      ...
      if (preparedSession.status === "replacement_required") {
        const choiceUrl = new URL("/login", `${req.protocol}://${req.get("host")}`);
        choiceUrl.searchParams.set("session_replace", preparedSession.sessionReplacementToken);
        choiceUrl.searchParams.set("returnTo", successRedirect);
        return res.redirect(choiceUrl.pathname + choiceUrl.search);
      }
  ...
  app.post("/api/auth/magic-verify", async (req: Request, res: Response) => {
    try {
      const { token, host: hostBody, deviceId } = req.body ?? {};
      if (!token) {
  ...
  app.get("/api/auth/access-verify", async (req: Request, res: Response) => {
    const { token, next, host: hostParam, deviceId } = req.query as Record<string, string>;
    if (!token) {
      return res.redirect("/auth/access-error?reason=missing_token");
    }
    try {
      const result = await completeAccessTokenLogin(req, res, String(token), hostParam, deviceId);
      if (result.status === "replacement_required") {
        const choiceUrl = new URL("/login", `${req.protocol}://${req.get("host")}`);
        choiceUrl.searchParams.set("session_replace", result.sessionReplacementToken);
        if (next?.startsWith("/")) choiceUrl.searchParams.set("returnTo", next);
        return res.redirect(choiceUrl.pathname + choiceUrl.search);
      }
  ...
  app.post("/api/auth/access-verify", async (req: Request, res: Response) => {
    try {
      ...
      const result = await completeAccessTokenLogin(
        req,
        res,
        String(token),
        undefined,
      ...
      if (result.status === "replacement_required") {
        return res.status(409).json({
          requiresSessionReplacement: true,
          sessionReplacementToken: result.sessionReplacementToken,
        });
      }
      if (result.status === "revoked") {
