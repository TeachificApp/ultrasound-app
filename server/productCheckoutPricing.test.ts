...
import { resolveUpgradeProductCheckoutCents } from "./routers/lmsRouter";
...
describe("digital and physical product checkout pricing", () => {
  ...
  it("uses the same audited conversion in LMS upgrade checkout paths", () => {
    expect(resolveUpgradeProductCheckoutCents("7.00")).toBe(700);
    expect(resolveUpgradeProductCheckoutCents("299.97")).toBe(29997);
  });
});
