...
describe("source-file generation routers", () => {
  it("passes an uploaded PDF to lmsAdmin.aiGenerateCourse and commits its final course assessment", async () => {
    ...
    const generated = await generateCaller.aiGenerateCourse({ topics: "Uploaded source", productType: "course", moduleCount: 3, lessonsPerModule: 3, sourceFile, generateCourseQuiz: true });
    ...
    expect((mocks.invokeLLM.mock.calls[0][0].messages[1].content as any)).toContainEqual(expect.objectContaining({ type: "file_url" }));
  ...
  it("passes an uploaded source to Question Bank generation and persists explanation and every option feedback", async () => {
    ...
    const result = await caller.aiGenerateToBank({ topic: "Source image", count: 1, sourceFile: { ...sourceFile, mimeType: "image/png", url: "https://files.example/source.png", name: "source.png" } });
    expect(result.inserted).toBe(1);
    expect(mocks.invokeLLM).toHaveBeenLastCalledWith(expect.objectContaining({ model: "gemini-3-flash-preview" }));
    expect((mocks.invokeLLM.mock.calls.at(-1)![0].messages[1].content as any)).toContainEqual(expect.objectContaining({ type: "image_url" }));
