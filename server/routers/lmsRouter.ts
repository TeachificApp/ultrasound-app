...
export const lmsGroupRouter = router({
  ...
  /** Send a test email to verify email provider delivery (admin only) */
  sendTestEmail: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const providerLabel = provider === "smtpcom" ? "SMTP.com" : "SendGrid";
      ...
      const htmlBody = emailWrapper(`
        <h2 style="margin:0 0 12px;font-size:20px;color:#0e1e2e;font-family:Georgia,serif;">
          ✅ ${providerLabel} Test Email
        </h2>
        <p style="margin:0 0 16px;font-size:15px;color:#475569;line-height:1.6;">
          This is a test email sent from the <strong>Platform Admin → Email Settings</strong> panel to verify that your ${providerLabel} integration is working correctly.
        </p>
        <div style="background:#f0fbfc;border-left:3px solid #189aa1;padding:14px 16px;border-radius:0 8px 8px 0;margin:0 0 20px;">
          ...
          <p style="margin:4px 0 0;font-size:13px;color:#475569;">Provider: <strong>${providerLabel}</strong></p>
        ...
        <p style="margin:0;font-size:13px;color:#94a3b8;">
          If you received this email, your ${providerLabel} configuration is working correctly. No action is needed.
        </p>
      `, brandMode);
      const ok = await sendEmail({
        to: { name: ctx.user.name ?? "Admin", email: input.recipientEmail },
        subject: `[Test] ${providerLabel} delivery check — ${now}`,
        htmlBody,
        brandMode,
      });
      if (!ok) throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: `${providerLabel} rejected the test email. Check the server logs for details (look for [email] ${provider} send failed).` });
      return { success: true, sentTo: input.recipientEmail, provider };
    }),
