    ...
    .query(async ({ ctx, input }) => {
      ...
      const [nativeMemberships] = await db.execute(sql`
        SELECT
          ...
          ms.cancel_at_period_end AS cancelAtPeriodEnd,
    ...
    .query(async ({ ctx, input }) => {
      ...
      return {
        emails: toArr2(rows).map((r: any) => ({
          ...
          sentAt: r.sentAt instanceof Date ? r.sentAt.toISOString() : (r.sentAt ? new Date(String(r.sentAt) + (String(r.sentAt).endsWith('Z') || String(r.sentAt).includes('+') ? '' : 'Z')).toISOString() : null),
