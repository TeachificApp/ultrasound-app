...
async function handlePhysicalProductCheckoutCompleted(session: Record<string, unknown>) {
  ...
  if (orderId) {
    ...
    try {
      const pfResult = await fulfillPrintifyOrder(db, orderId, {
        customerEmail: meta.customer_email ?? (session.customer_email as string) ?? null,
      });
      if (pfResult.submitted) {
        console.log(`[Stripe] Printify fulfillment started for order ${orderId}`);
      } else if (pfResult.error) {
        console.warn(`[Stripe] Printify fulfillment failed for order ${orderId}: ${pfResult.error}`);
      }
    } catch (err) {
      console.error(`[Stripe] Printify fulfillment error for order ${orderId}:`, err);
    }
  }
