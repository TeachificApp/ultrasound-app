...
describe("shared Social Post Library", () => {
  ...
  it("saves administrator-approved posts, shares same-brand records across platform admins, and validates repository image ownership", () => {
    ...
    expect(router).toContain("savePostToLibrary: adminProcedure");
  ...
  it("persists every editable Social Post text field and keeps category choices brand-safe", () => {
    ...
    expect(client).toContain("savePostToLibraryMutation");
    expect(client).toContain("Changes update the preview only");
    expect(client).toContain("Save to Post Library");
  ...
  it("keeps every generator theme compatible with the MySQL library enum", () => {
    ...
    expect(schema).toContain('cardTheme: mysqlEnum("cardTheme", ["dark", "light", "white", "teal", "aqua"])');
    expect(migration).toContain("MODIFY COLUMN `cardTheme` ENUM('dark', 'light', 'white', 'teal', 'aqua')");
  });
