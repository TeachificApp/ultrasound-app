...
const mocks = vi.hoisted(() => ({ courseGenerate: vi.fn(), questionGenerate: vi.fn(), addToQuiz: vi.fn(), generatedPayload: null as any }));
...
vi.mock("@/lib/trpc", () => ({
  trpc: {
    ...
    questionBank: {
      ...
      aiGenerateToBank: { useMutation: (options: any) => ({ mutate: (input: any) => { mocks.questionGenerate(input); options?.onSuccess?.(mocks.generatedPayload); }, mutateAsync: vi.fn(), isPending: false }) }, confirmScormImport: { useMutation: () => mutation() }, importCsvToBank: { useMutation: () => mutation() },
    },
    quizMaker: { getQuiz: { useQuery: () => ({ data: null, isLoading: false }) }, addQuestionBankQuestions: { useMutation: () => mutation(mocks.addToQuiz) } },
...
describe("source authoring dialogs", () => {
  ...
  afterEach(async () => { await act(async () => root?.unmount()); host?.remove(); globalThis.fetch = originalFetch; mocks.generatedPayload = null; vi.clearAllMocks(); });
  ...
  it("submits the Quiz Creator's remaining reviewed sources as a sourceFiles array", async () => {
    ...
    const countSelect = Array.from(document.querySelectorAll("select")).find(select => Array.from(select.options).some(option => option.value === "250")) as HTMLSelectElement;
    await act(async () => { countSelect.value = "250"; countSelect.dispatchEvent(new Event("change", { bubbles: true })); });
    await upload([new File(["one"], "one.pdf", { type: "application/pdf" }), new File(["two"], "two.png", { type: "image/png" })]);
    await act(async () => (document.querySelector('[aria-label="Remove two.png"]') as HTMLButtonElement).click());
    const generatedQuestions = Array.from({ length: 250 }, (_, index) => ({ id: index + 1000, question: `Generated question ${index + 1}`, type: index === 0 ? "multiselect" : "mcq", options: ["A", "B", "C", "D"], correctAnswer: "B", explanation: `Explanation ${index + 1}` }));
    mocks.generatedPayload = { ids: generatedQuestions.map(question => question.id), questions: generatedQuestions };
    ...
    expect(mocks.questionGenerate).toHaveBeenCalledWith(expect.objectContaining({ count: 250, sourceFiles: [{ url: "https://files.example/one.pdf", mimeType: "application/pdf", name: "one.pdf" }] }));
    await act(async () => Array.from(document.querySelectorAll("button")).find(button => button.textContent?.includes("Select all"))?.click());
    await act(async () => Array.from(document.querySelectorAll("button")).find(button => button.textContent?.includes("Add 250 to Quiz"))?.click());
    expect(mocks.addToQuiz).toHaveBeenCalledWith({ quizId: 5, questionBankIds: generatedQuestions.map(question => question.id), groupId: undefined });
  });
});
