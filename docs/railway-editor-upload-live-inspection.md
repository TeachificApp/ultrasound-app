...
Railway also accepted the subsequent default-off quiz read-aloud checkpoint. It is progressing through the normal production build while the prior Forge/voice release remains active. The updated release changes only new and previously unset quizzes to default off; it retains explicit creator choices and the Female/Male profiles. Wait for this newer deployment to become active before validating it in the live Quiz Creator and learner player.

The next production checkpoint changes the live AI fallback for the actual service configuration. The service has a Railway-held Manus API key but no complete Forge chat URL/key pair, so the lesson editor no longer forces Forge. It now selects automatic transport: Forge only when a complete Forge configuration later exists; otherwise, the Manus API is used through a private, lightweight, non-interactive task with no inherited connectors or skills. Railway is currently building this revision; live editor generation must be retried only after the deployment becomes active.

The subsequent checkpoint adds one safe recovery for the remaining authoring-task pause. It automatically replies only when the API identifies the waiting event as a normal agent question (`messageAskUser`), using a fixed no-tools and no-external-actions instruction. It does not act on any confirmation, browser, connector, email, deployment, payment, or other external-action wait. Railway is currently building this latest checkpoint while the prior healthy service remains active.

The user has now added an OpenAI-compatible server secret in Railway Production and explicitly confirmed applying the staged service configuration. Railway started a fresh application deployment. The shared transport resolver already selects a direct OpenAI chat completion whenever `OPENAI_API_KEY` is present, so no Manus task is used after the deployment is active. The secret value was not viewed, copied, or recorded.

The confirmed variable update is now rebuilding the currently active checkpoint in Railway Production. The application remains online on the prior revision until the fresh container passes health checks. Live lesson generation must be retried only after this configuration-triggered deployment is active.

Railway has completed the configuration-triggered deployment successfully. The current production container is now running with the OpenAI-compatible secret; the next authoring generation request should select direct chat completion before any Manus task fallback.

Railway production logs confirm that the direct authoring completion request now reaches the configured provider and receives HTTP 429. The prior Manus task confirmation error is no longer the active failure. The next repair must add bounded, server-side retry/backoff for rate-limit responses and present authors a clear retry state without returning provider error bodies or credential details.

The sandbox Railway browser session expired while checking the later course and lesson focus-regeneration deployment. The direct service route redirected to Railway’s login/404 state, so that browser could not independently reconfirm the active revision. This did not change Railway or production behavior. Live administrator validation should occur only after Railway is reauthenticated or the new authoring controls appear in the administrator interface.
