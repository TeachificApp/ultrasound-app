...
On August 22, the same preview module rate limit prevented a rendered `/auth/reset-password` form during legacy password-link verification: `src/index.css` and `src/lib/trpc.ts` returned HTTP 429. The reset-path helper regression and the full App shell bundle validate the redirect logic independently.
