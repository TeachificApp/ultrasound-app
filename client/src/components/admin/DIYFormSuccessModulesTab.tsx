/**
 * DIYFormSuccessModulesTab.tsx
 * Admin UI for configurable multi-path success pages for DIY/Accreditation forms.
 * Mirrors FormSuccessModulesTab but uses trpc.formBuilder.* procedures.
 */
...
export default function DIYFormSuccessModulesTab({
  templateId,
  template,
  onRefetch,
  formItems,
  ...
  const upsertModule = trpc.formBuilder.upsertSuccessModule.useMutation({
    onSuccess: () => { toast.success("Success module saved"); refetchModules(); onRefetch(); },
    onError: e => toast.error(e.message),
  });
  const duplicateModule = trpc.formBuilder.duplicateSuccessModule.useMutation({
    onSuccess: () => { toast.success("Module duplicated"); refetchModules(); },
    onError: e => toast.error(e.message),
  });
  const deleteModule = trpc.formBuilder.deleteSuccessModule.useMutation({
    onSuccess: () => { toast.success("Module deleted"); refetchModules(); onRefetch(); },
    onError: e => toast.error(e.message),
  });
  const setDefault = trpc.formBuilder.setDefaultSuccessModule.useMutation({
    onSuccess: () => { toast.success("Default module updated"); refetchModules(); onRefetch(); },
    onError: e => toast.error(e.message),
  });
  const upsertRule = trpc.formBuilder.upsertSuccessRoutingRule.useMutation({
    onSuccess: () => { toast.success("Routing rule saved"); refetchRules(); },
    onError: e => toast.error(e.message),
  });
  const deleteRule = trpc.formBuilder.deleteSuccessRoutingRule.useMutation({
    onSuccess: () => { toast.success("Rule deleted"); refetchRules(); },
    onError: e => toast.error(e.message),
  });
  const updateTemplate = trpc.formBuilder.updateTemplate.useMutation({
  ...
  const [previewModuleState, setPreviewModuleState] = useState<any | null>(null);
  ...
  const fieldOptions = useMemo(() => {
    const items = (formItems ?? []).filter((it) =>
      !["heading", "paragraph", "section_break", "rich_text", "info", "richtext"].includes(it.itemType),
    );
    return [
      ...SPECIAL_FIELDS,
      ...items.map((it) => ({ value: String(it.id), label: it.label || `Field #${it.id}` })),
    ];
  }, [formItems]);
  ...
  return (
    <div className="space-y-6">
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            <div>
              ...
              <Select
                ...
                onValueChange={v => setDefault.mutate({ templateId, moduleId: v === "none" ? null : parseInt(v, 10) })}
              >
            ...
            <div>
              ...
              <div className="flex gap-2 mt-1">
                ...
                <Button type="button" variant="outline" size="sm" onClick={savePassingScore} disabled={updateTemplate.isPending}>Save</Button>
              </div>
            </div>
          </div>
          ...
          <Card key={mod.id} className={!mod.isEnabled ? "opacity-60" : ""}>
            <CardContent className="py-3 flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-1">
                <Button type="button" size="sm" variant="ghost" className="h-8 gap-1" onClick={() => setPreviewModuleState(mod)}>
                ...
                {defaultModuleId !== mod.id && (
                  <Button type="button" size="sm" variant="ghost" className="h-8 text-amber-700" onClick={() => setDefault.mutate({ templateId, moduleId: mod.id })}>
                    Set default
                  </Button>
                )}
      ...
      {/* Preview Dialog */}
      <Dialog open={!!previewModuleState} onOpenChange={() => setPreviewModuleState(null)}>
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto p-0">
          {previewModuleState && (
            <FormSuccessOutcomeView outcome={previewSuccessModule(previewModuleState)} />
          )}
        </DialogContent>
      </Dialog>
            ...
            <div className="space-y-3">
              ...
              <div className="pt-2 border-t border-gray-100 space-y-3">
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id="diy-rule-stripe-enabled"
                    checked={ruleDraft?.stripeEnabled ?? false}
                    onChange={e => setRuleDraft(r => r ? { ...r, stripeEnabled: e.target.checked } : r)}
                    className="h-4 w-4 rounded border-gray-300"
                  />
                  <label htmlFor="diy-rule-stripe-enabled" className="text-sm font-medium text-gray-700">
