            ...
            <div className="text-center">
              <Avatar className="w-14 h-14 mx-auto mb-2 ring-2 ring-gray-300">
                ...
                <AvatarFallback className="bg-gray-100 text-gray-600 font-bold">
                  {publicMemberDisplayName(leaderboard[1].user ?? {}).charAt(0).toUpperCase()}
                </AvatarFallback>
              </Avatar>
              <div className="text-2xl mb-1">🥈</div>
              <p className="text-sm font-semibold text-gray-900 truncate max-w-[80px]">{publicMemberDisplayName(leaderboard[1].user ?? {})}</p>
            ...
            <div className="text-center -mt-4">
              <Avatar className="w-18 h-18 mx-auto mb-2 ring-4 ring-amber-400" style={{ width: 72, height: 72 }}>
                ...
                <AvatarFallback className="bg-amber-100 text-amber-700 font-bold text-xl">
                  {publicMemberDisplayName(leaderboard[0].user ?? {}).charAt(0).toUpperCase()}
                </AvatarFallback>
              </Avatar>
              <div className="text-3xl mb-1">🥇</div>
              <p className="text-sm font-bold text-gray-900 truncate max-w-[80px]">{publicMemberDisplayName(leaderboard[0].user ?? {})}</p>
            ...
            <div className="text-center">
              <Avatar className="w-14 h-14 mx-auto mb-2 ring-2 ring-amber-700/40">
                ...
                <AvatarFallback className="bg-amber-50 text-amber-700 font-bold">
                  {publicMemberDisplayName(leaderboard[2].user ?? {}).charAt(0).toUpperCase()}
                </AvatarFallback>
              </Avatar>
              <div className="text-2xl mb-1">🥉</div>
              <p className="text-sm font-semibold text-gray-900 truncate max-w-[80px]">{publicMemberDisplayName(leaderboard[2].user ?? {})}</p>
          ...
          <CardContent className="p-0">
            {isLoading ? (
              ...
              leaderboard.map((entry, i) => {
                ...
                return (
                  <Link key={entry.userId} href={`/community/members/${entry.userId}`}>
