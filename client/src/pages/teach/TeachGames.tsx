...
export default function TeachGames() {
  ...
  const requestedStudyGroupId = Number(new URLSearchParams(location.split("?")[1] || "").get("studyGroupId"));
  const studyGroupId = Number.isSafeInteger(requestedStudyGroupId) && requestedStudyGroupId > 0 ? requestedStudyGroupId : null;

  const { data: teachContext, isLoading: contextLoading } = trpc.teach.getMyContext.useQuery(undefined, { enabled: !!user });
  const { data: groupWorkspace, isLoading: groupLoading } = trpc.studyGroups.getWorkspace.useQuery(
    { groupId: studyGroupId! },
    { enabled: Boolean(user && studyGroupId) },
  );
  const canUseGroupTeach = Boolean(
    groupWorkspace?.group?.isOrganizationActive && groupWorkspace?.permissions?.canManage,
  );
  const canUseTeach = studyGroupId ? canUseGroupTeach : Boolean(teachContext?.canAccessTeach);
  const { data: allGames = [], isLoading: gamesLoading } = trpc.sonoQuiz.listQuizzes.useQuery(
    studyGroupId ? { studyGroupId } : undefined,
    { enabled: Boolean(canUseTeach) },
  );
  ...
  if (loading || contextLoading || Boolean(studyGroupId && groupLoading)) return <div className="min-h-screen grid place-items-center bg-slate-50"><Loader2 className="w-7 h-7 animate-spin text-teal-600" /></div>;
  if (!user) { window.location.href = getLoginUrl("/teach/games"); return null; }
  if (!canUseTeach) return <div className="min-h-screen grid place-items-center p-6 bg-slate-50"><Card className="max-w-md"><CardHeader><CardTitle>{studyGroupId ? "Organization admin access required" : "TEACH access required"}</CardTitle><CardDescription>{studyGroupId ? "Only active Organization Study Group owners and organization admins can create or manage this group’s TEACH activities." : "Live games are available to LMS instructors and EducatorAssist™ educators."}</CardDescription></CardHeader></Card></div>;
  ...
  const handleKahootImport = async (file: File) => {
    ...
    setImporting(true);
    importKahoot.mutate({ title: file.name.replace(/\.xlsx$/i, ""), fileName: file.name, fileData: await toBase64(file), ownerContext: "lms_instructor", ...(studyGroupId ? { studyGroupId } : {}) });
  };
  ...
  return <div className="min-h-screen bg-slate-50 text-slate-900">
    <header className="border-b bg-white"><div className="max-w-7xl mx-auto px-4 py-4 flex flex-wrap gap-3 items-center justify-between"><div className="flex items-center gap-3"><Link href={studyGroupId ? `/study-groups/${studyGroupId}` : "/teach"}><Button variant="ghost" size="icon" aria-label={studyGroupId ? "Back to Study Group" : "Back to Teach"}><ArrowLeft className="w-4 h-4" /></Button></Link><div><h1 className="text-xl font-bold flex items-center gap-2"><Gamepad2 className="text-teal-600" /> {studyGroupId ? `${groupWorkspace?.group?.name ?? "Organization"} TEACH Activities` : "Teach Live Games"}</h1><p className="text-sm text-slate-500">{studyGroupId ? "Create and launch your organization’s in-platform live learning activities." : "Teacher-led interactive sessions with QR code and Game PIN joining"}</p></div></div><div className="flex gap-2"><input ref={kahootRef} className="hidden" type="file" accept=".xlsx" onChange={(event) => { const file = event.target.files?.[0]; if (file) handleKahootImport(file); event.currentTarget.value = ""; }} /><Button variant="outline" onClick={() => kahootRef.current?.click()} disabled={importing}><Upload className="w-4 h-4 mr-1" /> {importing ? "Importing…" : "Import Kahoot .xlsx"}</Button><Button className="bg-teal-600 hover:bg-teal-700" onClick={() => setNewGameOpen(true)}><Plus className="w-4 h-4 mr-1" /> New live game</Button></div></div></header>
    <main className="max-w-7xl mx-auto px-4 py-6">
      {!selectedGameId ? <section>{gamesLoading ? <div className="py-20 text-center"><Loader2 className="mx-auto animate-spin text-teal-600" /></div> : games.length === 0 ? <Card className="border-dashed"><CardContent className="py-16 text-center"><Sparkles className="w-10 h-10 text-aqua-500 mx-auto mb-3" /><h2 className="font-semibold">{studyGroupId ? "Create your first group TEACH activity" : "Create your first live teaching game"}</h2><p className="text-sm text-slate-500 mt-1">Mix quiz, word cloud, hotspot, and puzzle slides—or import an authorised Kahoot spreadsheet.</p></CardContent></Card> : <div className="grid sm:grid-cols-2 xl:grid-cols-3 gap-4">{games.map((game: any) => <Card key={game.id} className="overflow-hidden"><div className="h-2 bg-teal-600" /><CardHeader><div className="flex gap-2 justify-between"><Badge variant="outline" className="border-teal-200 text-teal-700">Live game</Badge><Badge variant="outline">{game.status}</Badge></div><CardTitle className="mt-3">{game.title}</CardTitle><CardDescription>{game.questionCount} slide{game.questionCount === 1 ? "" : "s"} · {game.timeLimitSeconds}s default</CardDescription></CardHeader><CardContent className="flex gap-2"><Button className="flex-1" variant="outline" onClick={() => setSelectedGameId(game.id)}>Edit slides</Button><Button className="bg-teal-600 hover:bg-teal-700" disabled={!game.questionCount} onClick={() => launchGame.mutate({ quizId: game.id, allowAnonymous: true, showLeaderboard: true })}><Play className="w-4 h-4" /></Button></CardContent></Card>)}</div>}</section> : <section>
    ...
    <Dialog open={newGameOpen} onOpenChange={setNewGameOpen}><DialogContent><DialogHeader><DialogTitle>{studyGroupId ? "New group TEACH activity" : "New Teach live game"}</DialogTitle></DialogHeader><div className="space-y-4"><div><Label>Game title</Label><Input value={gameTitle} onChange={(event) => setGameTitle(event.target.value)} placeholder="e.g., Fetal Echo Warm-Up" className="mt-1" /></div><div><Label>Description</Label><Textarea value={gameDescription} onChange={(event) => setGameDescription(event.target.value)} placeholder="Optional teacher notes" className="mt-1" /></div></div><DialogFooter><Button variant="outline" onClick={() => setNewGameOpen(false)}>Cancel</Button><Button className="bg-teal-600 hover:bg-teal-700" disabled={!gameTitle.trim() || createGame.isPending} onClick={() => createGame.mutate({ title: gameTitle.trim(), description: gameDescription || undefined, isTeachGame: true, ownerContext: "lms_instructor", importSource: "manual", ...(studyGroupId ? { studyGroupId } : {}) })}>Create game</Button></DialogFooter></DialogContent></Dialog>
