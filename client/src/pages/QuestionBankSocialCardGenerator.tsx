...
export default function QuestionBankSocialCardGenerator() {
  ...
  const saveCardMutation = trpc.quizCardLibrary.save.useMutation({ onSuccess: () => { void utils.quizCardLibrary.list.invalidate(); toast.success("Saved to the shared Quiz Card Library."); } });
    ...
    try {
      const filename = await exportSocialCard({
        ...
        motion: cardVariant === "answer"
          ? { kind: "answer", title: activeQuestion.question, options, detail: "Review the question", answer: correctAnswer, brandName: presentation.displayName, accentColor: presentation.accentColor, logoUrl: presentation.logoUrl, musicUrl: selectedMusic?.url, musicTitle: selectedMusic?.title }
          : cardVariant === "combined"
            ? { kind: "combined", title: activeQuestion.question, options, answer: correctAnswer, brandName: presentation.displayName, accentColor: presentation.accentColor, logoUrl: presentation.logoUrl, musicUrl: selectedMusic?.url, musicTitle: selectedMusic?.title }
            : { kind: "question", title: activeQuestion.question, options, brandName: presentation.displayName, accentColor: presentation.accentColor, logoUrl: presentation.logoUrl, musicUrl: selectedMusic?.url, musicTitle: selectedMusic?.title },
      });
  ...
  }, [activeQuestion, cardVariant, correctAnswer, exportFormat, exportPlatform, options, presentation.accentColor, presentation.displayName, presentation.logoUrl, selectedMusic?.title, selectedMusic?.url]);
