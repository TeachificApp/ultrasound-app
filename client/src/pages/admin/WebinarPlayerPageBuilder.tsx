/**
 * WebinarPlayerPageBuilder.tsx
 * Full-screen drag-and-drop WYSIWYG page editor.
 * Route: /admin/webinars/:webinarId/player-page-builder
 *
...
export default function WebinarPlayerPageBuilder() {
  const { webinarId } = useParams<{ webinarId: string }>();
  const [, navigate] = useLocation();
  const numericId = Number(webinarId);
  ...
  // ── Load data ──
  const { isLoading, data: lpData } = trpc.webinarAdmin.getById.useQuery(
    { id: numericId },
  ...
  useEffect(() => {
    ...
    setHasLoaded(true);
    setPageInfo({ title: lpData.title + " — Player Page", slug: lpData.slug });
    // playerPageBlocks stored in salesPageBlocksJson for now — use a dedicated field if available
    const raw = (lpData as any).playerPageBlocks;
    if (raw) {
      try { setBlocks(JSON.parse(raw) as Block[]); } catch {}
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lpData]);

  // ── Save blocks ──
  const saveBlocks = trpc.webinarAdmin.update.useMutation({
    onSuccess: () => toast.success("Player page saved!"),
    onError: (e: any) => toast.error(`Save failed: ${e.message}`),
  });
  ...
  const handleSaveSeo = () => {
    toast.info('SEO settings are managed on the landing page.');
  };

  const handleSave = async () => {
    setIsSaving(true);
    try {
      await saveBlocks.mutateAsync({ id: numericId, playerPageBlocks: JSON.stringify(blocks) } as any);
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-white border-b border-gray-200 shadow-sm flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-gray-400">Webinar Player / Watch Page</p>
