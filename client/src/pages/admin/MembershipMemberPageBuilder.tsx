/**
 * MembershipMemberPageBuilder.tsx
 * Full-screen drag-and-drop WYSIWYG page editor.
 * Route: /admin/memberships/:planId/member-page-builder
 *
...
export default function MembershipMemberPageBuilder() {
  ...
  useEffect(() => {
    ...
    setPageInfo({ title: plan.title + " — Member Page", slug: plan.slug });
    if (plan.memberPageBlocks) {
      try { setBlocks(JSON.parse(plan.memberPageBlocks) as Block[]); } catch {}
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lpData]);

  // ── Save blocks ──
  const saveBlocks = trpc.membership.updateMemberPageBlocks.useMutation({
    onSuccess: () => toast.success("Member page saved!"),
    onError: (e: any) => toast.error(`Save failed: ${e.message}`),
  });
  ...
  const handleSaveSeo = () => {
    toast.info('SEO settings are managed on the sales page.');
  };
  ...
  const previewPath = pageInfo?.slug ? `/my-memberships/${pageInfo.slug}` : null;
    ...
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-white border-b border-gray-200 shadow-sm flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-gray-400">Membership Member Page</p>
