/**
 * WorkshopInstanceLandingPageBuilder.tsx
 * Full-screen drag-and-drop WYSIWYG page editor for individual workshop instance detail pages.
 * Route: /admin/workshops/:workshopId/instances/:instanceId/page-builder
 *
...
export default function WorkshopInstanceLandingPageBuilder() {
  const { workshopId, instanceId } = useParams<{ workshopId: string; instanceId: string }>();
  ...
  // Load instance landing blocks
  const { isLoading, data: instanceData } = trpc.workshopAdmin.getInstanceLandingBlocks.useQuery(
    { instanceId: numericInstanceId },
    { enabled: !isNaN(numericInstanceId) }
  );
  // Load workshop for back-link
  const { data: workshopData } = trpc.workshopAdmin.getById.useQuery(
    { id: numericWorkshopId },
    { enabled: !isNaN(numericWorkshopId) }
  );

  useEffect(() => {
    if (!instanceData || hasLoaded) return;
    setHasLoaded(true);
    setPageInfo({ title: instanceData.title });
    if (instanceData.landingBlocks) {
      try { setBlocks(JSON.parse(instanceData.landingBlocks) as Block[]); } catch {}
    }
  ...
  useEffect(() => {
    if (workshopData?.workshop) {
      setPageInfo(prev => prev ? { ...prev, workshopSlug: workshopData.workshop.slug } : prev);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [workshopData]);

  const saveBlocksMutation = trpc.workshopAdmin.saveInstanceLandingBlocks.useMutation({
    onSuccess: () => toast.success("Instance page saved!"),
    onError: (e: any) => toast.error(`Save failed: ${e.message}`),
  });
  ...
  const handleSave = async () => {
    setIsSaving(true);
    try {
      await saveBlocksMutation.mutateAsync({ instanceId: numericInstanceId, blocks: JSON.stringify(blocks) });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="flex flex-col h-screen bg-gray-100 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-white border-b shadow-sm z-10 shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <span className="text-xs text-gray-400">Instance Page:</span>
        ...
        <div className="flex-1 overflow-y-auto bg-gray-100 p-4">
          {blocks.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-gray-400 gap-4">
              ...
              <p className="text-sm">Build the detail page for this workshop instance</p>
            </div>
          ) : (
          ...
          {selectedBlock ? (
            ...
            <div className="p-4 space-y-4">
              ...
              <p className="text-xs text-gray-400 text-center">
                Build the full detail page for this workshop instance. Visitors see this when they click the instance card.
              </p>
              <div className="text-xs text-gray-400 text-center pt-2">Click a block to edit its settings</div>
            </div>
          )}
        </div>
      </div>
