/**
 * MembershipSalesPageBuilder.tsx
 * Full-screen drag-and-drop WYSIWYG page editor.
 * Route: /admin/memberships/:planId/sales-page-builder
 *
...
export default function MembershipSalesPageBuilder() {
  ...
  // ── Load data ──
  const { isLoading, data: lpData } = trpc.membership.getById.useQuery(
    { id: numericId },
  ...
  useEffect(() => {
    ...
    setHasLoaded(true);
    const plan = lpData.plan;
    setPageInfo({ title: plan.title, slug: plan.slug });
    if (plan.landingPageBlocks) {
      try { setBlocks(JSON.parse(plan.landingPageBlocks) as Block[]); } catch {}
    }
    if (plan.seoTitle) setSeoTitle(plan.seoTitle);
    if (plan.seoDescription) setSeoDescription(plan.seoDescription);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lpData]);

  // ── Save blocks ──
  const saveBlocks = trpc.membership.updateLandingPageBlocks.useMutation({
    onSuccess: () => toast.success("Sales page saved!"),
    onError: (e: any) => toast.error(`Save failed: ${e.message}`),
  });

  // ── Save SEO ──
  // No dedicated SEO procedure for memberships — saved via updateLandingPageBlocks
  const seoSavePending = false;

  const handleSaveSeo = () => {
    toast.info('SEO settings will be saved with the page blocks.');
  };

  const handleSave = async () => {
    setIsSaving(true);
    try {
      await saveBlocks.mutateAsync({ id: numericId, blocks: JSON.stringify(blocks) });
    } finally {
      setIsSaving(false);
    }
  };
    ...
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-white border-b border-gray-200 shadow-sm flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-gray-400">Membership Sales Page</p>
