/**
 * WebinarLandingPageBuilder.tsx
 * Full-screen drag-and-drop WYSIWYG page editor.
 * Route: /admin/webinars/:webinarId/landing-builder
 *
...
export default function WebinarLandingPageBuilder() {
  ...
  useEffect(() => {
    ...
    if (lpData.metaTitle) setSeoTitle(lpData.metaTitle);
    if (lpData.metaDescription) setSeoDescription(lpData.metaDescription);
    if (lpData.seoImage) setSeoImage(lpData.seoImage);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lpData]);

  // ── Save blocks ──
  const saveBlocks = trpc.webinarAdmin.update.useMutation({
    onSuccess: () => toast.success("Webinar landing page saved!"),
    onError: (e: any) => toast.error(`Save failed: ${e.message}`),
  });

  // ── Save SEO ──
  const saveSeoMutation = trpc.webinarAdmin.update.useMutation({
  ...
  const previewPath = pageInfo?.slug ? `/webinars/${pageInfo.slug}?preview=admin` : null;
    ...
    <div className="flex flex-col h-screen bg-gray-50 overflow-hidden">
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-white border-b border-gray-200 shadow-sm flex-shrink-0">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs text-gray-400">Webinar Landing Page</p>
