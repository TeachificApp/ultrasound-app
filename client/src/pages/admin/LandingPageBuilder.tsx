                  ...
                  {(btn.behavior ?? "url") === "direct_checkout" && (
                    <div className="space-y-2 bg-teal-50 border border-teal-200 rounded p-2">
                      ...
                      <div><label className="text-xs text-gray-500 block mb-0.5">Product</label>
                        <select value={btn.checkoutProductId ? `${btn.checkoutProductType}:${btn.checkoutProductId}` : ""} onChange={e => { const [type, id] = e.target.value.split(":"); setBtn(idx, "checkoutProductType", type); setBtn(idx, "checkoutProductId", id ? Number(id) : undefined); }} className="w-full h-7 text-xs rounded border border-gray-200 px-2">
                          <option value="">-- Select product --</option>
                          {(productCatalog ?? []).map(p => <option key={`${p.type}:${p.id}`} value={`${p.type}:${p.id}`}>{p.name} ({p.type}) — ${(p.price / 100).toFixed(2)}</option>)}
                        </select>
                      </div>
                      <div><label className="text-xs text-gray-500 block mb-0.5">Promo Code (optional)</label><DebouncedInput value={btn.checkoutPromoCode ?? ""} onChange={v => setBtn(idx, "checkoutPromoCode", v)} className="h-7 text-xs" placeholder="e.g. SAVE20" /></div>
                    </div>
                  )}
        ...
        <div className="space-y-3">
          ...
          {(d.ctaBehavior ?? "url") === "direct_checkout" && (
            <div className="space-y-2 bg-teal-50 border border-teal-200 rounded p-2">
              <p className="text-[10px] text-teal-700 font-medium">Opens Stripe Checkout. After payment, user is sent to /my-dashboard.</p>
              <div><label className="text-xs text-gray-500 block mb-0.5">Product</label>
                <select value={d.checkoutProductId ? `${d.checkoutProductType}:${d.checkoutProductId}` : ""} onChange={e => { const [type, id] = e.target.value.split(":"); set("checkoutProductType", type); set("checkoutProductId", id ? Number(id) : undefined); }} className="w-full h-7 text-xs rounded border border-gray-200 px-2">
                  <option value="">-- Select product --</option>
                  {(productCatalog ?? []).map(p => <option key={`${p.type}:${p.id}`} value={`${p.type}:${p.id}`}>{p.name} ({p.type}) — ${(p.price / 100).toFixed(2)}</option>)}
                </select>
              </div>
              <div><label className="text-xs text-gray-500 block mb-0.5">Promo Code (optional)</label><DebouncedInput value={d.checkoutPromoCode ?? ""} onChange={v => set("checkoutPromoCode", v)} className="h-7 text-xs" placeholder="e.g. SAVE20" /></div>
            </div>
          )}
