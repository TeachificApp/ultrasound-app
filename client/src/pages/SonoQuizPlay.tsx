...
import { getLiveSessionBrand } from "@shared/teachLiveSessionPresentation";
...
export default function SonoQuizPlay() {
  ...
  const joinSession = trpc.sonoQuiz.joinSession.useMutation({
    onSuccess: (data) => {
      ...
      setQuizInfo({ title: data.quizTitle, musicTrack: data.musicTrack, theme: data.theme, isTeachGame: data.isTeachGame });
      setPhase("lobby");
    },
    onError: (err) => {
